...
class Bird:
    ...
    def move(self):
        ...
        if d < 0 or self.y < self.height + 50:
            if self.titl < self.MAX_ROTATION:
                self.titl = self.MAX_ROTATION
        else:
            if self.titl > -90:
                self.titl -= self.ROT_VEL
    ...
    def draw(self, win):
        ...
        rotated_image = pygame.transform.rotate(self.img, self.titl)
...
def draw_window(win, birds, pipes, base, score, gen):
    ...
    # gen
    text = STAT_FONT.render("Tries: " + str(gen), 1, (255, 255, 255))
    win.blit(text, (10, 10))
...
if __name__ == "__main__":
    ...
    config_path = os.path.join(local_dir, "config-freedorward.txt")
    run(config_path)
